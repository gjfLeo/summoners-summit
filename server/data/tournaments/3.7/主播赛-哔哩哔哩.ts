import { AratakiItto, Barbara, Bennett, Chongyun, Collei, Cyno, ElectroHypostasis, FatuiPyroAgent, Fischl, Ganyu, HuTao, Jean, KamisatoAyaka, Keqing, Klee, KujouSara, Mona, Nahida, Ningguang, Noelle, RaidenShogun, RhodeiaOfLoch, Shenhe, Tartaglia, Tighnari, Venti, Xingqiu, YaeMiko, Yoimiya, Zhongli, defineActions, defineTournament } from "~/utils/types";

const actions = {
  "卡密sama42-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
  }),
  "卡密sama42-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "天守阁": 2,
    "珊瑚宫": 1,
    "须弥城": 1,
    "桓纳兰那": 1,
    "立本": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 2,
    "运筹帷幄": 1,
    "交给我吧！": 1,
    "白垩之术": 1,
    "送你一程": 1,
    "护法之誓": 1,
    "重攻击": 2,
    "莲花酥": 2,
    "烤蘑菇披萨": 2,
  }),
  "卡密sama42-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "拉娜": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "原皮鸭-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "原皮鸭-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "天守阁": 2,
    "桓纳兰那": 1,
    "立本": 2,
    "寻宝仙灵": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 1,
    "一掷乾坤": 1,
    "交给我吧！": 1,
    "送你一程": 2,
    "重攻击": 2,
    "温妮莎传奇": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 2,
  }),
  "原皮鸭-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "立本": 2,
    "寻宝仙灵": 1,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 1,
    "换班时间": 2,
    "一掷乾坤": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "卡牌狂魔张老伯-水债皇": defineActions({
    "悉数讨回": 2,
    "幸运儿银冠": 1,
    "赌徒的耳环": 1,
    "璃月港口": 1,
    "天守阁": 1,
    "桓纳兰那": 1,
    "派蒙": 2,
    "提米": 1,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
    "烤蘑菇披萨": 2,
  }),
  "卡牌狂魔张老伯-火云鹤": defineActions({
    "冒险憧憬": 2,
    "祭礼剑": 2,
    "赌徒的耳环": 2,
    "璃月港口": 2,
    "桓纳兰那": 1,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "最好的伙伴！": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "莲花酥": 2,
    "北地烟熏鸡": 2,
    "蒙德土豆饼": 2,
  }),
  "卡牌狂魔张老伯-莫温刻": defineActions({
    "祭礼剑": 2,
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "鲸井小弟": 2,
    "寻宝仙灵": 2,
    "风与自由": 2,
    "最好的伙伴！": 1,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "白垩之术": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "唐杜尔烤鸡": 2,
  }),

  "回忆往昔的遗憾-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
  }),
  "回忆往昔的遗憾-火刻": defineActions({
    "冒险憧憬": 2,
    "祭礼剑": 1,
    "赌徒的耳环": 2,
    "璃月港口": 2,
    "须弥城": 2,
    "派蒙": 2,
    "立本": 2,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "风与自由": 2,
    "最好的伙伴！": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "送你一程": 2,
    "莲花酥": 2,
    "唐杜尔烤鸡": 1,
  }),
  "回忆往昔的遗憾-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 1,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 1,
  }),

  "原来是筱梦-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "拉娜": 1,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "原来是筱梦-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "月话牌社-音月-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "最好的伙伴！": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
  }),
  "月话牌社-音月-双火": defineActions({
    "冒险憧憬": 2,
    "赌徒的耳环": 2,
    "璃月港口": 2,
    "派蒙": 2,
    "立本": 2,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "风与自由": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "送你一程": 1,
    "护法之誓": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "月话牌社-音月-水皇草": defineActions({
    "幸运儿银冠": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 1,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "Glyou_HZ-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "Glyou_HZ-双火": defineActions({
    "冒险憧憬": 2,
    "祭礼剑": 2,
    "赌徒的耳环": 2,
    "璃月港口": 2,
    "派蒙": 2,
    "立本": 2,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "风与自由": 2,
    "最好的伙伴！": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "送你一程": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "唐杜尔烤鸡": 1,
  }),
  "Glyou_HZ-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "八重妞妞-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "天守阁": 2,
    "桓纳兰那": 2,
    "寻宝仙灵": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "送你一程": 1,
    "重攻击": 1,
    "温妮莎传奇": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 2,
  }),
  "八重妞妞-水债皇": defineActions({
    "悉数讨回": 1,
    "幸运儿银冠": 1,
    "璃月港口": 2,
    "天守阁": 2,
    "桓纳兰那": 1,
    "派蒙": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 2,
    "送你一程": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 2,
  }),

  "七圣召唤王师傅-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 1,
    "送你一程": 1,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "七圣召唤王师傅-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "立本": 2,
    "拉娜": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 1,
    "蒙德土豆饼": 2,
  }),
  "七圣召唤王师傅-莫温刻": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "拉娜": 2,
    "寻宝仙灵": 2,
    "风与自由": 2,
    "最好的伙伴！": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "星天之兆": 1,
    "白垩之术": 1,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "唐杜尔烤鸡": 1,
  }),

  "阿源今晚吃派蒙-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "刘苏": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "阿源今晚吃派蒙-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "须弥城": 2,
    "桓纳兰那": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 1,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "交给我吧！": 1,
    "白垩之术": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 2,
  }),
  "阿源今晚吃派蒙-水皇草": defineActions({
    "幸运儿银冠": 1,
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "零之启-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 1,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
  }),
  "零之启-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "西风大教堂": 1,
    "天守阁": 2,
    "桓纳兰那": 2,
    "立本": 1,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 1,
    "运筹帷幄": 1,
    "本大爷还没有输！": 1,
    "交给我吧！": 1,
    "送你一程": 2,
    "重攻击": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 2,
  }),
  "零之启-水皇草": defineActions({
    "幸运儿银冠": 1,
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "派蒙": 1,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "圣刻-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "寻宝仙灵": 1,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "圣刻-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "齿轮mog-双冰": defineActions({
    "赌徒的耳环": 2,
    "璃月港口": 2,
    "桓纳兰那": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 2,
    "一掷乾坤": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "北地烟熏鸡": 1,
    "蒙德土豆饼": 2,
  }),
  "齿轮mog-双雷": defineActions({
    "幸运儿银冠": 1,
    "璃月港口": 2,
    "派蒙": 2,
    "立本": 2,
    "元素共鸣：交织之雷": 2,
    "元素共鸣：强能之雷": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "白垩之术": 1,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "齿轮mog-莫琴刻": defineActions({
    "祭礼剑": 2,
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "寻宝仙灵": 2,
    "风与自由": 2,
    "最好的伙伴！": 1,
    "换班时间": 2,
    "一掷乾坤": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 1,
    "蒙德土豆饼": 2,
    "唐杜尔烤鸡": 1,
  }),

  "蓬莱寺九霄啊-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "寻宝仙灵": 1,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
  }),
  "蓬莱寺九霄啊-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "天守阁": 2,
    "须弥城": 1,
    "桓纳兰那": 1,
    "立本": 1,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 1,
    "交给我吧！": 1,
    "送你一程": 2,
    "护法之誓": 1,
    "重攻击": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 1,
  }),
  "蓬莱寺九霄啊-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "柠柠七喵-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "寻宝仙灵": 1,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "柠柠七喵-双火": defineActions({
    "冒险憧憬": 2,
    "赌徒的耳环": 2,
    "璃月港口": 2,
    "桓纳兰那": 1,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "柠柠七喵-水债皇": defineActions({
    "悉数讨回": 1,
    "幸运儿银冠": 2,
    "璃月港口": 2,
    "天守阁": 1,
    "桓纳兰那": 1,
    "派蒙": 2,
    "提米": 1,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 2,
    "白垩之术": 1,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 1,
  }),
  "幽羊-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 1,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
  }),
  "幽羊-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 1,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "唐傀-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "寻宝仙灵": 1,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
  }),
  "唐傀-双火": defineActions({
    "冒险憧憬": 2,
    "祭礼剑": 2,
    "赌徒的耳环": 2,
    "璃月港口": 1,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "风与自由": 2,
    "最好的伙伴！": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
    "唐杜尔烤鸡": 1,
  }),
  "唐傀-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "派蒙": 1,
    "提米": 2,
    "立本": 2,
    "拉娜": 1,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "赤佬RedY-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "拉娜": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 1,
  }),
  "赤佬RedY-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "天守阁": 2,
    "珊瑚宫": 2,
    "须弥城": 2,
    "阿圆": 2,
    "立本": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "运筹帷幄": 2,
    "交给我吧！": 1,
    "白垩之术": 1,
    "送你一程": 1,
    "护法之誓": 1,
    "莲花酥": 2,
    "烤蘑菇披萨": 2,
  }),

  "小铭同学qaQ233-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
  }),
  "小铭同学qaQ233-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "天守阁": 2,
    "须弥城": 2,
    "立本": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 2,
    "最好的伙伴！": 1,
    "换班时间": 1,
    "运筹帷幄": 1,
    "交给我吧！": 1,
    "送你一程": 2,
    "重攻击": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "小铭同学qaQ233-水皇草": defineActions({
    "赌徒的耳环": 1,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "魔法绵羊Tobby-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 1,
    "提米": 2,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "魔法绵羊Tobby-双草": defineActions({
    "赌徒的耳环": 2,
    "提米": 2,
    "立本": 1,
    "寻宝仙灵": 2,
    "元素共鸣：交织之草": 2,
    "元素共鸣：蔓生之草": 2,
    "草与智慧": 2,
    "最好的伙伴！": 2,
    "换班时间": 1,
    "一掷乾坤": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 1,
    "重攻击": 1,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "魔法绵羊Tobby-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "梦一夕林-双火": defineActions({
    "冒险憧憬": 2,
    "赌徒的耳环": 2,
    "璃月港口": 2,
    "鸣神大社": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "立本": 2,
    "参量质变仪": 2,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "莲花酥": 2,
    "北地烟熏鸡": 2,
    "兽肉薄荷卷": 2,
  }),
  "梦一夕林-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "天守阁": 2,
    "桓纳兰那": 1,
    "立本": 1,
    "寻宝仙灵": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 1,
    "送你一程": 2,
    "重攻击": 1,
    "温妮莎传奇": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
    "烤蘑菇披萨": 2,
  }),
  "梦一夕林-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 1,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "zjyang-双冰": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "提米": 1,
    "立本": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "岩与契约": 1,
    "换班时间": 1,
    "一掷乾坤": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "zjyang-双岩": defineActions({
    "钟剑": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "天守阁": 2,
    "桓纳兰那": 2,
    "立本": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "风与自由": 2,
    "运筹帷幄": 2,
    "交给我吧！": 1,
    "送你一程": 2,
    "护法之誓": 1,
    "重攻击": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
    "烤蘑菇披萨": 1,
  }),
  "zjyang-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 1,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "音尘Voicend-双冰": defineActions({
    "赌徒的耳环": 2,
    "提米": 2,
    "立本": 2,
    "刘苏": 2,
    "拉娜": 2,
    "寻宝仙灵": 2,
    "元素共鸣：交织之冰": 2,
    "元素共鸣：粉碎之冰": 2,
    "雷与永恒": 2,
    "换班时间": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "送你一程": 2,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),
  "音尘Voicend-双火": defineActions({
    "冒险憧憬": 2,
    "赌徒的耳环": 2,
    "璃月港口": 1,
    "桓纳兰那": 1,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "参量质变仪": 1,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "风与自由": 2,
    "运筹帷幄": 1,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "温妮莎传奇": 1,
    "莲花酥": 2,
    "北地烟熏鸡": 2,
    "蒙德土豆饼": 1,
  }),
  "音尘Voicend-水皇草": defineActions({
    "赌徒的耳环": 2,
    "桓纳兰那": 2,
    "派蒙": 2,
    "提米": 2,
    "立本": 2,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 1,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "星天之兆": 1,
    "白垩之术": 2,
    "送你一程": 1,
    "仙跳墙": 1,
    "莲花酥": 2,
    "蒙德土豆饼": 2,
  }),

  "RR抒情夜莺-双火": defineActions({
    "冒险憧憬": 2,
    "祭礼剑": 1,
    "赌徒的耳环": 2,
    "须弥城": 2,
    "派蒙": 2,
    "提米": 1,
    "立本": 2,
    "拉娜": 2,
    "元素共鸣：交织之火": 2,
    "元素共鸣：热诚之火": 2,
    "风与自由": 1,
    "最好的伙伴！": 2,
    "本大爷还没有输！": 2,
    "交给我吧！": 2,
    "送你一程": 1,
    "莲花酥": 2,
    "唐杜尔烤鸡": 2,
  }),
  "RR抒情夜莺-双岩": defineActions({
    "贯虹之槊": 2,
    "千岩牢固": 2,
    "璃月港口": 2,
    "西风大教堂": 2,
    "珊瑚宫": 2,
    "须弥城": 2,
    "阿圆": 2,
    "立本": 2,
    "元素共鸣：交织之岩": 2,
    "元素共鸣：坚定之岩": 2,
    "本大爷还没有输！": 1,
    "交给我吧！": 1,
    "送你一程": 2,
    "重攻击": 1,
    "永远的友谊": 1,
    "莲花酥": 2,
    "烤蘑菇披萨": 2,
  }),
  "RR抒情夜莺-莫皇草": defineActions({
    "赌徒的耳环": 2,
    "须弥城": 2,
    "提米": 2,
    "拉娜": 2,
    "风与自由": 1,
    "最好的伙伴！": 2,
    "换班时间": 2,
    "一掷乾坤": 2,
    "运筹帷幄": 2,
    "本大爷还没有输！": 2,
    "星天之兆": 1,
    "白垩之术": 1,
    "送你一程": 2,
    "仙跳墙": 2,
    "莲花酥": 2,
    "蒙德土豆饼": 1,
    "唐杜尔烤鸡": 2,
  }),
};

export default defineTournament({
  name: "哔哩哔哩主播资格赛",
  type: "主播资格赛",
  gameVersion: "3.7",
  stages: [
    {
      name: "小组赛",
      rules: ["BO3", "决斗"],
      parts: [
        {
          name: "A组",
          date: "2023-06-27",
          matches: [
            {
              playerA: "水瓶子",
              playerB: "卡密sama42",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1pu411h7Hg",
              games: [
                {
                  playerACharacters: [Bennett, Keqing, FatuiPyroAgent],
                  playerAActions: defineActions({
                    "冒险憧憬": 2,
                    "悉数讨回": 1,
                    "祭礼剑": 1,
                    "赌徒的耳环": 2,
                    "璃月港口": 1,
                    "须弥城": 2,
                    "派蒙": 2,
                    "提米": 2,
                    "刘苏": 1,
                    "寻宝仙灵": 1,
                    "元素共鸣：交织之火": 2,
                    "元素共鸣：热诚之火": 2,
                    "最好的伙伴！": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                    "唐杜尔烤鸡": 1,
                  }),
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["卡密sama42-双冰"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [YaeMiko, Tighnari, Collei],
                  playerAActions: defineActions({
                    "赌徒的耳环": 2,
                    "提米": 2,
                    "立本": 2,
                    "刘苏": 1,
                    "寻宝仙灵": 1,
                    "元素共鸣：交织之草": 2,
                    "元素共鸣：蔓生之草": 2,
                    "草与智慧": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 2,
                    "一掷乾坤": 1,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "快快缝补术": 1,
                    "重攻击": 2,
                    "仙跳墙": 1,
                    "莲花酥": 2,
                    "唐杜尔烤鸡": 1,
                  }),
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["卡密sama42-水皇草"],
                  starter: "A",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "企鹅带带北极熊",
              playerB: "卡牌狂魔张老伯",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1pu411h7Hg?p=2",
              games: [
                {
                  playerACharacters: [Chongyun, HuTao, Shenhe],
                  playerAActions: defineActions({
                    "血之灶火": 2,
                    "千岩长枪": 2,
                    "赌徒的耳环": 2,
                    "立本": 2,
                    "刘苏": 2,
                    "元素共鸣：交织之冰": 2,
                    "元素共鸣：粉碎之冰": 2,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "星天之兆": 2,
                    "送你一程": 2,
                    "重攻击": 2,
                    "温妮莎传奇": 2,
                    "北地烟熏鸡": 2,
                    "兽肉薄荷卷": 2,
                  }),
                  playerBCharacters: [RhodeiaOfLoch, Fischl, FatuiPyroAgent],
                  playerBActions: actions["卡牌狂魔张老伯-水债皇"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Klee, RhodeiaOfLoch, Nahida],
                  playerAActions: defineActions({
                    "砰砰礼物": 2,
                    "千夜浮梦": 2,
                    "赌徒的耳环": 2,
                    "桓纳兰那": 2,
                    "立本": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 2,
                    "一掷乾坤": 2,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "温妮莎传奇": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                    "唐杜尔烤鸡": 2,
                  }),
                  playerBCharacters: [Bennett, Chongyun, Shenhe],
                  playerBActions: actions["卡牌狂魔张老伯-火云鹤"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Tartaglia, Bennett, FatuiPyroAgent],
                  playerAActions: defineActions({
                    "冒险憧憬": 2,
                    "赌徒的耳环": 2,
                    "璃月港口": 1,
                    "派蒙": 2,
                    "提米": 2,
                    "立本": 2,
                    "拉娜": 2,
                    "元素共鸣：交织之火": 2,
                    "元素共鸣：热诚之火": 2,
                    "最好的伙伴！": 2,
                    "运筹帷幄": 1,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "愚人众的阴谋": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 1,
                    "唐杜尔烤鸡": 1,
                  }),
                  playerBCharacters: [Keqing, Mona, Venti],
                  playerBActions: actions["卡牌狂魔张老伯-莫温刻"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "原皮鸭",
              playerB: "BattleSpt",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1pu411h7Hg?p=3",
              games: [
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["原皮鸭-双岩"],
                  playerBCharacters: [Ningguang, YaeMiko, Zhongli],
                  playerBActions: defineActions({
                    "贯虹之槊": 2,
                    "千岩牢固": 2,
                    "璃月港口": 2,
                    "西风大教堂": 2,
                    "珊瑚宫": 2,
                    "须弥城": 2,
                    "阿圆": 2,
                    "立本": 2,
                    "元素共鸣：交织之岩": 2,
                    "元素共鸣：坚定之岩": 2,
                    "本大爷还没有输！": 1,
                    "送你一程": 2,
                    "重攻击": 1,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                    "烤蘑菇披萨": 2,
                  }),
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["原皮鸭-双冰"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: defineActions({
                    "赌徒的耳环": 2,
                    "桓纳兰那": 2,
                    "派蒙": 2,
                    "提米": 2,
                    "立本": 2,
                    "拉娜": 2,
                    "最好的伙伴！": 2,
                    "一掷乾坤": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 1,
                    "星天之兆": 1,
                    "白垩之术": 2,
                    "送你一程": 2,
                    "仙跳墙": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                  }),
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "回忆往昔的遗憾",
              playerB: "九方羽幻",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1pu411h7Hg?p=4",
              games: [
                {
                  playerACharacters: [Fischl, RhodeiaOfLoch, Nahida],
                  playerAActions: actions["回忆往昔的遗憾-水皇草"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: defineActions({
                    "赌徒的耳环": 2,
                    "桓纳兰那": 2,
                    "立本": 2,
                    "寻宝仙灵": 1,
                    "元素共鸣：交织之冰": 2,
                    "元素共鸣：粉碎之冰": 2,
                    "岩与契约": 1,
                    "最好的伙伴！": 2,
                    "换班时间": 2,
                    "一掷乾坤": 1,
                    "运筹帷幄": 1,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "送你一程": 2,
                    "仙跳墙": 1,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                  }),
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["回忆往昔的遗憾-双冰"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: defineActions({
                    "赌徒的耳环": 1,
                    "桓纳兰那": 2,
                    "派蒙": 2,
                    "提米": 2,
                    "立本": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 2,
                    "一掷乾坤": 2,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 1,
                    "星天之兆": 1,
                    "白垩之术": 2,
                    "送你一程": 1,
                    "仙跳墙": 1,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                    "提瓦特煎蛋": 1,
                  }),
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Bennett, Klee, Keqing],
                  playerAActions: actions["回忆往昔的遗憾-火刻"],
                  playerBCharacters: [Barbara, AratakiItto, Noelle],
                  playerBActions: defineActions({
                    "钟剑": 2,
                    "千岩牢固": 2,
                    "天守阁": 2,
                    "桓纳兰那": 1,
                    "立本": 2,
                    "寻宝仙灵": 2,
                    "元素共鸣：交织之岩": 2,
                    "元素共鸣：坚定之岩": 2,
                    "风与自由": 1,
                    "最好的伙伴！": 1,
                    "交给我吧！": 1,
                    "送你一程": 2,
                    "重攻击": 2,
                    "温妮莎传奇": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                    "烤蘑菇披萨": 2,
                  }),
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "卡密sama42",
              playerB: "卡牌狂魔张老伯",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1pu411h7Hg?p=5",
              games: [
                {
                  playerACharacters: [Noelle, Barbara, AratakiItto],
                  playerAActions: actions["卡密sama42-双岩"],
                  playerBCharacters: [Keqing, Mona, Venti],
                  playerBActions: actions["卡牌狂魔张老伯-莫温刻"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["卡密sama42-水皇草"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, FatuiPyroAgent],
                  playerBActions: actions["卡牌狂魔张老伯-水债皇"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "原皮鸭",
              playerB: "回忆往昔的遗憾",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1pu411h7Hg?p=6",
              games: [
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["原皮鸭-水皇草"],
                  playerBCharacters: [Fischl, RhodeiaOfLoch, Nahida],
                  playerBActions: actions["回忆往昔的遗憾-水皇草"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["原皮鸭-双冰"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["回忆往昔的遗憾-双冰"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "原皮鸭",
              playerB: "卡密sama42",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1pu411h7Hg?p=7",
              games: [
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["原皮鸭-水皇草"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["卡密sama42-双冰"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Barbara, AratakiItto, Noelle],
                  playerAActions: actions["原皮鸭-双岩"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["卡密sama42-水皇草"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["原皮鸭-双冰"],
                  playerBCharacters: [Noelle, Barbara, AratakiItto],
                  playerBActions: actions["卡密sama42-双岩"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
          ],
        },
        {
          name: "B组",
          date: "2023-06-28",
          matches: [
            {
              playerA: "原来是筱梦",
              playerB: "今天不想吃苹果_",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Mh4y1M7vW",
              games: [
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["原来是筱梦-水皇草"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: defineActions({
                    "赌徒的耳环": 1,
                    "桓纳兰那": 2,
                    "提米": 2,
                    "立本": 2,
                    "拉娜": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 2,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "白垩之术": 2,
                    "送你一程": 2,
                    "仙跳墙": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                    "烤蘑菇披萨": 1,
                  }),
                  starter: "A",
                  winner: "A",
                  turns: 3,
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["原来是筱梦-双冰"],
                  playerBCharacters: [RaidenShogun, Shenhe, Chongyun],
                  playerBActions: defineActions({
                    "赌徒的耳环": 2,
                    "桓纳兰那": 1,
                    "提米": 2,
                    "立本": 2,
                    "元素共鸣：交织之冰": 2,
                    "元素共鸣：粉碎之冰": 2,
                    "岩与契约": 1,
                    "最好的伙伴！": 2,
                    "换班时间": 1,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "送你一程": 2,
                    "仙跳墙": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                  }),
                  starter: "A",
                  winner: "A",
                },
              ],
            },
            {
              // cSpell:word Glyou_HZ
              playerA: "Glyou_HZ",
              playerB: "芭芭拉教主",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Mh4y1M7vW?p=2",
              games: [
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["Glyou_HZ-双冰"],
                  playerBCharacters: [Shenhe, RaidenShogun, Chongyun],
                  playerBActions: defineActions({
                    "赌徒的耳环": 2,
                    "璃月港口": 2,
                    "桓纳兰那": 2,
                    "提米": 2,
                    "立本": 2,
                    "元素共鸣：交织之冰": 2,
                    "元素共鸣：粉碎之冰": 2,
                    "岩与契约": 1,
                    "最好的伙伴！": 2,
                    "运筹帷幄": 1,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "送你一程": 2,
                    "仙跳墙": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 1,
                  }),
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Bennett, Keqing, Klee],
                  playerAActions: actions["Glyou_HZ-双火"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: defineActions({
                    "赌徒的耳环": 2,
                    "桓纳兰那": 2,
                    "派蒙": 2,
                    "立本": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 2,
                    "一掷乾坤": 2,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "白垩之术": 2,
                    "送你一程": 2,
                    "仙跳墙": 1,
                    "莲花酥": 2,
                    "蒙德土豆饼": 2,
                  }),
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["Glyou_HZ-水皇草"],
                  playerBCharacters: [Collei, Tighnari, YaeMiko],
                  playerBActions: defineActions({
                    "阿莫斯之弓": 1,
                    "赌徒的耳环": 2,
                    "立本": 1,
                    "元素共鸣：交织之草": 2,
                    "元素共鸣：蔓生之草": 2,
                    "草与智慧": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 1,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "白垩之术": 1,
                    "快快缝补术": 1,
                    "送你一程": 2,
                    "重攻击": 1,
                    "仙跳墙": 2,
                    "莲花酥": 2,
                    "蒙德土豆饼": 1,
                  }),
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "八重妞妞",
              playerB: "｜蓝兰岚｜",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Mh4y1M7vW?p=3",
              games: [
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, FatuiPyroAgent],
                  playerAActions: actions["八重妞妞-水债皇"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: defineActions({
                    "幸运儿银冠": 1,
                    "赌徒的耳环": 1,
                    "天守阁": 1,
                    "桓纳兰那": 1,
                    "派蒙": 1,
                    "提米": 1,
                    "立本": 2,
                    "拉娜": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 1,
                    "一掷乾坤": 2,
                    "运筹帷幄": 1,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "白垩之术": 2,
                    "送你一程": 2,
                    "仙跳墙": 2,
                    "蒙德土豆饼": 2,
                    "烤蘑菇披萨": 1,
                  }),
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Barbara, AratakiItto, Noelle],
                  playerAActions: actions["八重妞妞-双岩"],
                  playerBCharacters: [Venti, Mona, Keqing],
                  playerBActions: defineActions({
                    "祭礼剑": 1,
                    "赌徒的耳环": 2,
                    "桓纳兰那": 2,
                    "提米": 2,
                    "鲸井小弟": 2,
                    "寻宝仙灵": 2,
                    "风与自由": 2,
                    "最好的伙伴！": 1,
                    "换班时间": 2,
                    "一掷乾坤": 1,
                    "运筹帷幄": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "白垩之术": 2,
                    "仙跳墙": 1,
                    "莲花酥": 2,
                    "蒙德土豆饼": 1,
                    "唐杜尔烤鸡": 2,
                  }),
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              // cSpell:word 夜梦永怀音理-Nera
              playerA: "月话牌社-音月",
              playerB: "夜梦永怀音理-Nera",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Mh4y1M7vW?p=4",
              games: [
                {
                  playerACharacters: [Bennett, Klee, ElectroHypostasis],
                  playerAActions: actions["月话牌社-音月-双火"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: defineActions({
                    "幸运儿银冠": 2,
                    "桓纳兰那": 2,
                    "派蒙": 2,
                    "提米": 1,
                    "立本": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 2,
                    "一掷乾坤": 2,
                    "运筹帷幄": 2,
                    "本大爷还没有输！": 2,
                    "交给我吧！": 2,
                    "星天之兆": 1,
                    "白垩之术": 2,
                    "仙跳墙": 1,
                    "莲花酥": 2,
                    "蒙德土豆饼": 1,
                    "烤蘑菇披萨": 1,
                    "提瓦特煎蛋": 1,
                  }),
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["月话牌社-音月-双冰"],
                  playerBCharacters: [Ganyu, Xingqiu, Mona],
                  playerBActions: defineActions({
                    "唯此一心": 2,
                    "阿莫斯之弓": 2,
                    "赌徒的耳环": 1,
                    "派蒙": 1,
                    "立本": 2,
                    "刘苏": 2,
                    "元素共鸣：交织之水": 2,
                    "元素共鸣：愈疗之水": 2,
                    "岩与契约": 2,
                    "最好的伙伴！": 2,
                    "换班时间": 1,
                    "一掷乾坤": 2,
                    "运筹帷幄": 1,
                    "星天之兆": 1,
                    "送你一程": 1,
                    "重攻击": 2,
                    "莲花酥": 2,
                    "烤蘑菇披萨": 1,
                    "提瓦特煎蛋": 1,
                  }),
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "Glyou_HZ",
              playerB: "原来是筱梦",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Mh4y1M7vW?p=5",
              games: [
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["Glyou_HZ-水皇草"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["原来是筱梦-水皇草"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["Glyou_HZ-双冰"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["原来是筱梦-双冰"],
                  starter: "A",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "月话牌社-音月",
              playerB: "八重妞妞",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Mh4y1M7vW?p=6",
              games: [
                {
                  playerACharacters: [Bennett, Klee, ElectroHypostasis],
                  playerAActions: actions["月话牌社-音月-双火"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, FatuiPyroAgent],
                  playerBActions: actions["八重妞妞-水债皇"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["月话牌社-音月-水皇草"],
                  playerBCharacters: [Barbara, AratakiItto, Noelle],
                  playerBActions: actions["八重妞妞-双岩"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "Glyou_HZ",
              playerB: "月话牌社-音月",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1Mh4y1M7vW?p=7",
              games: [
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["Glyou_HZ-双冰"],
                  playerBCharacters: [Bennett, Klee, ElectroHypostasis],
                  playerBActions: actions["月话牌社-音月-双火"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["Glyou_HZ-水皇草"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["月话牌社-音月-水皇草"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Bennett, Keqing, Klee],
                  playerAActions: actions["Glyou_HZ-双火"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["月话牌社-音月-双冰"],
                  starter: "A",
                  winner: "B",
                },
              ],
            },
          ],
        },
        {
          name: "C组",
          date: "2023-06-29",
          matches: [
            {
              playerA: "阿源今晚吃派蒙",
              playerB: "零之启",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Ea4y1c7z6",
              games: [
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["阿源今晚吃派蒙-水皇草"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["零之启-水皇草"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["阿源今晚吃派蒙-双冰"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["零之启-双冰"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [AratakiItto, Barbara, Noelle],
                  playerAActions: actions["阿源今晚吃派蒙-双岩"],
                  playerBCharacters: [Barbara, AratakiItto, Noelle],
                  playerBActions: actions["零之启-双岩"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "圣刻",
              playerB: "齿轮mog",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1Ea4y1c7z6?p=2",
              games: [
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["圣刻-双冰"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Cyno],
                  playerBActions: actions["齿轮mog-双雷"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["圣刻-水皇草"],
                  playerBCharacters: [Mona, Jean, Keqing],
                  playerBActions: actions["齿轮mog-莫琴刻"],
                  starter: "A",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "蓬莱寺九霄啊",
              playerB: "柠柠七喵",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Ea4y1c7z6?p=3",
              games: [
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["蓬莱寺九霄啊-双冰"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["柠柠七喵-双冰"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [AratakiItto, Barbara, Noelle],
                  playerAActions: actions["蓬莱寺九霄啊-双岩"],
                  playerBCharacters: [ElectroHypostasis, Bennett, Yoimiya],
                  playerBActions: actions["柠柠七喵-双火"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["蓬莱寺九霄啊-水皇草"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, FatuiPyroAgent],
                  playerBActions: actions["柠柠七喵-水债皇"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "幽羊",
              playerB: "七圣召唤王师傅",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1Ea4y1c7z6?p=4",
              games: [
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["幽羊-水皇草"],
                  playerBCharacters: [Mona, Venti, Keqing],
                  playerBActions: actions["七圣召唤王师傅-莫温刻"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["幽羊-双冰"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["七圣召唤王师傅-水皇草"],
                  starter: "A",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "阿源今晚吃派蒙",
              playerB: "齿轮mog",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1Ea4y1c7z6?p=5",
              games: [
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["阿源今晚吃派蒙-水皇草"],
                  playerBCharacters: [Mona, Jean, Keqing],
                  playerBActions: actions["齿轮mog-莫琴刻"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["阿源今晚吃派蒙-双冰"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["齿轮mog-双冰"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "蓬莱寺九霄啊",
              playerB: "七圣召唤王师傅",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1Ea4y1c7z6?p=6",
              games: [
                {
                  playerACharacters: [AratakiItto, Barbara, Noelle],
                  playerAActions: actions["蓬莱寺九霄啊-双岩"],
                  playerBCharacters: [Mona, Venti, Keqing],
                  playerBActions: actions["七圣召唤王师傅-莫温刻"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["蓬莱寺九霄啊-水皇草"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["七圣召唤王师傅-水皇草"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["蓬莱寺九霄啊-双冰"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["七圣召唤王师傅-双冰"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "阿源今晚吃派蒙",
              playerB: "七圣召唤王师傅",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1Ea4y1c7z6?p=7",
              games: [
                {
                  playerACharacters: [AratakiItto, Barbara, Noelle],
                  playerAActions: actions["阿源今晚吃派蒙-双岩"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["七圣召唤王师傅-水皇草"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["阿源今晚吃派蒙-双冰"],
                  playerBCharacters: [Mona, Venti, Keqing],
                  playerBActions: actions["七圣召唤王师傅-莫温刻"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["阿源今晚吃派蒙-水皇草"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["七圣召唤王师傅-双冰"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
          ],
        },
        {
          name: "D组",
          date: "2023-06-30",
          matches: [
            {
              playerA: "唐傀",
              playerB: "赤佬RedY",
              video: "https://www.bilibili.com/video/BV1eg4y1A7Qk",
              games: [
                {
                  playerACharacters: [Bennett, Keqing, Klee],
                  playerAActions: actions["唐傀-双火"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["赤佬RedY-水皇草"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["唐傀-双冰"],
                  playerBCharacters: [Barbara, AratakiItto, Noelle],
                  playerBActions: actions["赤佬RedY-双岩"],
                  starter: "A",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "小铭同学qaQ233",
              playerB: "魔法绵羊Tobby",
              video: "https://www.bilibili.com/video/BV1eg4y1A7Qk?p=2",
              games: [
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["小铭同学qaQ233-双冰"],
                  playerBCharacters: [Tighnari, YaeMiko, Collei],
                  playerBActions: actions["魔法绵羊Tobby-双草"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["小铭同学qaQ233-水皇草"],
                  playerBCharacters: [Shenhe, RaidenShogun, Chongyun],
                  playerBActions: actions["魔法绵羊Tobby-双冰"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Noelle, Barbara, AratakiItto],
                  playerAActions: actions["小铭同学qaQ233-双岩"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["魔法绵羊Tobby-水皇草"],
                  starter: "A",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "梦一夕林",
              playerB: "zjyang",
              video: "https://www.bilibili.com/video/BV1eg4y1A7Qk?p=3",
              games: [
                {
                  playerACharacters: [Bennett, FatuiPyroAgent, Cyno],
                  playerAActions: actions["梦一夕林-双火"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["zjyang-双冰"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Barbara, AratakiItto, Noelle],
                  playerAActions: actions["梦一夕林-双岩"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["zjyang-水皇草"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["梦一夕林-水皇草"],
                  playerBCharacters: [Noelle, Barbara, AratakiItto],
                  playerBActions: actions["zjyang-双岩"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "音尘Voicend",
              playerB: "RR抒情夜莺",
              video: "https://www.bilibili.com/video/BV1eg4y1A7Qk?p=4",
              games: [
                {
                  playerACharacters: [Bennett, Klee, Cyno],
                  playerAActions: actions["音尘Voicend-双火"],
                  playerBCharacters: [Bennett, Klee, Keqing],
                  playerBActions: actions["RR抒情夜莺-双火"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [KujouSara, Shenhe, KamisatoAyaka],
                  playerAActions: actions["音尘Voicend-双冰"],
                  playerBCharacters: [Fischl, Mona, Nahida],
                  playerBActions: actions["RR抒情夜莺-莫皇草"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["音尘Voicend-水皇草"],
                  playerBCharacters: [YaeMiko, Zhongli, Ningguang],
                  playerBActions: actions["RR抒情夜莺-双岩"],
                  starter: "A",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "唐傀",
              playerB: "魔法绵羊Tobby",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1eg4y1A7Qk?p=5",
              games: [
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["唐傀-水皇草"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["魔法绵羊Tobby-水皇草"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["唐傀-双冰"],
                  playerBCharacters: [Tighnari, YaeMiko, Collei],
                  playerBActions: actions["魔法绵羊Tobby-双草"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Bennett, Keqing, Klee],
                  playerAActions: actions["唐傀-双火"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["魔法绵羊Tobby-双冰"],
                  starter: "A",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "梦一夕林",
              playerB: "音尘Voicend",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1eg4y1A7Qk?p=6",
              games: [
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["梦一夕林-水皇草"],
                  playerBCharacters: [KujouSara, Shenhe, KamisatoAyaka],
                  playerBActions: actions["音尘Voicend-双冰"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Barbara, AratakiItto, Noelle],
                  playerAActions: actions["梦一夕林-双岩"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["音尘Voicend-水皇草"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "音尘Voicend",
              playerB: "魔法绵羊Tobby",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1eg4y1A7Qk?p=7",
              games: [
                {
                  playerACharacters: [Bennett, Klee, Cyno],
                  playerAActions: actions["音尘Voicend-双火"],
                  playerBCharacters: [Tighnari, YaeMiko, Collei],
                  playerBActions: actions["魔法绵羊Tobby-双草"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [KujouSara, Shenhe, KamisatoAyaka],
                  playerAActions: actions["音尘Voicend-双冰"],
                  playerBCharacters: [Shenhe, RaidenShogun, Chongyun],
                  playerBActions: actions["魔法绵羊Tobby-双冰"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["音尘Voicend-水皇草"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["魔法绵羊Tobby-水皇草"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
          ],
        },
      ],
    },
    {
      name: "淘汰赛",
      rules: ["BO3", "决斗"],
      parts: [
        {
          name: "8进4",
          date: "2023-07-01",
          matches: [
            {
              playerA: "七圣召唤王师傅",
              playerB: "卡密sama42",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX",
              games: [
                {
                  playerACharacters: [Mona, Venti, Keqing],
                  playerAActions: actions["七圣召唤王师傅-莫温刻"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["卡密sama42-双冰"],
                  starter: "B",
                  winner: "B",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["七圣召唤王师傅-水皇草"],
                  playerBCharacters: [Noelle, Barbara, AratakiItto],
                  playerBActions: actions["卡密sama42-双岩"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["七圣召唤王师傅-双冰"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["卡密sama42-水皇草"],
                  starter: "A",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "月话牌社-音月",
              playerB: "Glyou_HZ",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX?p=2",
              games: [
                {
                  playerACharacters: [Bennett, Klee, ElectroHypostasis],
                  playerAActions: actions["月话牌社-音月-双火"],
                  playerBCharacters: [Bennett, Keqing, Klee],
                  playerBActions: actions["Glyou_HZ-双火"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["月话牌社-音月-水皇草"],
                  playerBCharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerBActions: actions["Glyou_HZ-水皇草"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["月话牌社-音月-双冰"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["Glyou_HZ-双冰"],
                  starter: "A",
                  winner: "A",
                },
              ],
            },
            {
              playerA: "阿源今晚吃派蒙",
              playerB: "音尘Voicend",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX?p=3",
              games: [
                {
                  playerACharacters: [AratakiItto, Barbara, Noelle],
                  playerAActions: actions["阿源今晚吃派蒙-双岩"],
                  playerBCharacters: [KujouSara, Shenhe, KamisatoAyaka],
                  playerBActions: actions["音尘Voicend-双冰"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["阿源今晚吃派蒙-双冰"],
                  playerBCharacters: [Bennett, Klee, Cyno],
                  playerBActions: actions["音尘Voicend-双火"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["阿源今晚吃派蒙-水皇草"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["音尘Voicend-水皇草"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "原皮鸭",
              playerB: "魔法绵羊Tobby",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX?p=4",
              games: [
                {
                  playerACharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerAActions: actions["原皮鸭-水皇草"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["魔法绵羊Tobby-双冰"],
                  starter: "A",
                  winner: "A",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["原皮鸭-双冰"],
                  playerBCharacters: [Tighnari, YaeMiko, Collei],
                  playerBActions: actions["魔法绵羊Tobby-双草"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
          ],
        },
        {
          name: "半决赛",
          date: "2023-07-01",
          matches: [
            {
              playerA: "七圣召唤王师傅",
              playerB: "月话牌社-音月",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX?p=5",
              games: [
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["七圣召唤王师傅-水皇草"],
                  playerBCharacters: [Bennett, Klee, ElectroHypostasis],
                  playerBActions: actions["月话牌社-音月-双火"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Mona, Venti, Keqing],
                  playerAActions: actions["七圣召唤王师傅-莫温刻"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["月话牌社-音月-水皇草"],
                  starter: "A",
                  winner: "B",
                },
              ],
            },
            {
              playerA: "音尘Voicend",
              playerB: "原皮鸭",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX?p=6",
              games: [
                {
                  playerACharacters: [Bennett, Klee, Cyno],
                  playerAActions: actions["音尘Voicend-双火"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["原皮鸭-水皇草"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [KujouSara, Shenhe, KamisatoAyaka],
                  playerAActions: actions["音尘Voicend-双冰"],
                  playerBCharacters: [Barbara, AratakiItto, Noelle],
                  playerBActions: actions["原皮鸭-双岩"],
                  starter: "B",
                  winner: "A",
                },
              ],
            },
          ],
        },
        {
          name: "季军赛",
          date: "2023-07-01",
          matches: [
            {
              playerA: "七圣召唤王师傅",
              playerB: "原皮鸭",
              winner: "A",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX?p=7",
              games: [
                {
                  playerACharacters: [Mona, Venti, Keqing],
                  playerAActions: actions["七圣召唤王师傅-莫温刻"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["原皮鸭-水皇草"],
                  starter: "B",
                  winner: "A",
                },
                {
                  playerACharacters: [Nahida, RhodeiaOfLoch, Fischl],
                  playerAActions: actions["七圣召唤王师傅-水皇草"],
                  playerBCharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerBActions: actions["原皮鸭-双冰"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["七圣召唤王师傅-双冰"],
                  playerBCharacters: [Barbara, AratakiItto, Noelle],
                  playerBActions: actions["原皮鸭-双岩"],
                  starter: "A",
                  winner: "A",
                },
              ],
            },
          ],
        },
        {
          name: "决赛",
          date: "2023-07-01",
          matches: [
            {
              playerA: "月话牌社-音月",
              playerB: "音尘Voicend",
              winner: "B",
              video: "https://www.bilibili.com/video/BV1js4y1c7XX?p=8",
              games: [
                {
                  playerACharacters: [Bennett, Klee, ElectroHypostasis],
                  playerAActions: actions["月话牌社-音月-双火"],
                  playerBCharacters: [RhodeiaOfLoch, Fischl, Nahida],
                  playerBActions: actions["音尘Voicend-水皇草"],
                  starter: "A",
                  winner: "B",
                },
                {
                  playerACharacters: [Chongyun, RaidenShogun, Shenhe],
                  playerAActions: actions["月话牌社-音月-双冰"],
                  playerBCharacters: [KujouSara, Shenhe, KamisatoAyaka],
                  playerBActions: actions["音尘Voicend-双冰"],
                  starter: "B",
                  winner: "B",
                },
              ],
            },
          ],
        },
      ],
    },
  ],
});
