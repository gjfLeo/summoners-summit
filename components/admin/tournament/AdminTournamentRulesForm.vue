...
const numGames = computed<number | undefined>({
  get: () => {
    return rules.value?.numGames;
  },
  set: (v) => {
    if (rules.value && v) {
      rules.value.numGames = v;
    }
  },
});
const mode = computed<TournamentRules["mode"] | undefined>({
  get: () => {
    return rules.value?.mode;
  },
  set: (v) => {
    if (rules.value && v) {
      rules.value.mode = v;
